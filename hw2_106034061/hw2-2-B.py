...
#    h, status = cv2.findHomography(pts_src, pts_dst);
    h = Compute_Projection_Matrix(pts_src, pts_dst)

#    im_temp = cv2.warpPerspective(im_src, h, (im_dst.shape[1],im_dst.shape[0]))
    im_temp_F = Forward_Warpping(im_src, h, (im_dst.shape[1],im_dst.shape[0]))
#    cv2.imshow("Forward Warpping_"+str(index), im_temp_F);
    im_cut_F = im_temp_F.copy()
    cv2.fillConvexPoly(im_cut_F, pts_dst.astype(int), 0, 16);
    im_remain_F = im_temp_F - im_cut_F
    
    im_result_F = im_dst.copy()
    cv2.fillConvexPoly(im_result_F, pts_dst.astype(int), 0, 16);
    im_result_F = im_result_F + im_remain_F;
    cv2.imshow("Result_B_ForwardWaping_"+str(index), im_result_F);
    cv2.imwrite("./hw2-2/Result_B_ForwardWaping_"+str(index)+".jpg", im_result_F);
    
    im_temp_B = Backward_Warpping(im_src, h, (im_dst.shape[1],im_dst.shape[0]))
#    cv2.imshow("Backward Warpping_"+str(index), im_temp_B);
    im_cut_B = im_temp_B.copy()
    cv2.fillConvexPoly(im_cut_B, pts_dst.astype(int), 0, 16);
    im_remain_B = im_temp_B - im_cut_B
    im_result_B = im_dst.copy()
    
    cv2.fillConvexPoly(im_result_B, pts_dst.astype(int), 0, 16);
    im_result_B = im_result_B + im_remain_B;
    cv2.imshow("Result_B_BackwordWarping_"+str(index), im_result_B);
    cv2.imwrite("./hw2-2/Result_B_BackwordWarping_"+str(index)+".jpg", im_result_B);
    cv2.waitKey(0);
    
if __name__ == '__main__' :
    # Read source image.
    im_src = cv2.imread('./hw2-2/data_2-2_B_1.jpg');
    # Read destination image
    im_dst = cv2.imread('./hw2-2/data_2-2_B_0.jpg');
    if os.path.isfile('./hw2-2/pts_src_B.npy'):
        pts_src = np.load('./hw2-2/pts_src_B.npy')
    else:
        pts_src = Get_Four_Corner(im_src)
        np.save('hw2-2/pts_src_B', np.asarray(pts_src))
    if os.path.isfile('./hw2-2/pts_dst_B.npy'):
        pts_dst = np.load('./hw2-2/pts_dst_B.npy')
    else:
        ...
        np.save('hw2-2/pts_dst_B', np.asarray(pts_dst))
    Switch_Picture(pts_src, pts_dst, im_src, im_dst, 0)
    Switch_Picture(pts_dst, pts_src, im_dst, im_src, 1)
